package com.launchdarkly.chart.blankchart;
...
public class BlankPlotChart extends JComponent {
    ...
    public void setMaxValues(double newMaxValues) {
        // Store the target value
        this.targetMaxValues = newMaxValues;
        
        // If going to zero, keep current scale until animation completes
        if (newMaxValues <= 0.01 && maxValues > 0.01) {
            // Scale will be updated when animation completes
            return;
        }
    ...
    public void setAnimateProgress(float progress) {
        this.animateProgress = progress;
        // When animation completes, update scale if needed
        if (progress >= 1f && targetMaxValues <= 0.01) {
            this.maxValues = targetMaxValues;
            niceScale.setMax(maxValues);
        }
        repaint();
    }
    ...
    public void initValues(double minValues, double maxValues) {
        ...
        this.targetMaxValues = maxValues;
    ...
    private void createLine(Graphics2D g2) {
        ...
        // When all values are effectively zero and animation is complete
        if (targetMaxValues <= 0.01 && animateProgress >= 1f) {
    ...
    private void createValues(Graphics2D g2) {
        ...
        // When all values are effectively zero and animation is complete
        if (targetMaxValues <= 0.01 && animateProgress >= 1f) {
            // Draw only a single "0" at the bottom
            String text = "0";
