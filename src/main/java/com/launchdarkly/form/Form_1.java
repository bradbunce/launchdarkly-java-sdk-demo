package com.launchdarkly.form;

import com.launchdarkly.chart.ModelChart;
import static com.launchdarkly.config.LD.FEATURE_FLAG_1_KEY;
import static com.launchdarkly.config.LD.FEATURE_FLAG_2_KEY;
import static com.launchdarkly.config.LD.FEATURE_FLAG_3_KEY;
import static com.launchdarkly.config.LD.showMessage;
import com.launchdarkly.config.LD;
...
import javax.swing.*;
import com.launchdarkly.chart.Chart;
import com.launchdarkly.chart.LineChart;
import com.launchdarkly.swing.progress.Progress;
import com.launchdarkly.swing.RoundPanel;
...
public class Form_1 extends JPanel {
    ...
    private void updateProgressBars(boolean enabled) {
        try {
            // Setting value to 0 effectively resets the progress bar
            progress1.setValue(enabled ? PROGRESS_VALUES[0] : 0);
            progress2.setValue(enabled ? PROGRESS_VALUES[1] : 0);
            progress3.setValue(enabled ? PROGRESS_VALUES[2] : 0);
            
            // Only need to call start() when enabled, as setValue(0) handles the reset
            if (enabled) {
                progress1.start();
                progress2.start();
                progress3.start();
